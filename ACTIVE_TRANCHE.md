...
13. Current EP compare publication has zero true formula mismatches across the current 49-destination machine-facing EP export compare: 16 exact matches, 9 close matches, and 24 stage-scope rows documenting EP shortcuts or missing user-specific export inputs rather than simulator formula failures. EP `health_raw` still compares against the QE-published raw-health helper before the Armor-slot module health multiplier, while the existing post-armor health pool remains available for wall/eHP consumers. eDamage helper locals for Smart Missiles heatup, Poison Swamp Death Creep, ILM Charged Mines, and Chrono Field+ map to QE-published helper surfaces; locked or unowned UW helper deltas remain stage-scope rather than hidden calibration.
