...
36. COMPLETE/UPDATED: wire the first workbook GC-loadout mechanics into Boss Waves without copying workbook shortcuts: `GC Max Waves` routes Boss Waves stat primitives through the Tourney loadout, while farming-named presets (`eHP Farming`, `GC Farming`) and eHP max-wave presets route through Farming; QE-published Chain Lightning DPS now feeds a QE-owned CL-only boss-applicable support lane consumed as fail-closed continuous boss damage; Energy Net duration and Energy Net Mastery are KB/QE-backed Boss Waves primitives; and the evaluator applies the Energy Net multiplier through a generic time-bounded continuous-damage lane rather than card-specific kernel branching.
...
44. COMPLETE/UPDATED: map the workbook CL/EP boss-DPS bridge to explicit simulator inputs without copying workbook defaults. Boss Waves accepts a complete decomposed eDamage-to-boss bridge factor set (`boss_edamage_target_share`, `boss_edamage_cadence_uptime_factor`, `boss_edamage_reliability_factor`, `boss_edamage_semantic_normalizer`) and multiplies QE `derived::edamage` only when all four are supplied; partial decomposed bridge inputs fail closed with a ValueError. No Dissonant Run category gets a stronger default boss-damage source than regular runs: defaults remain on the QE-owned CL-only boss-applicable lane unless an explicit bridge is supplied. Selected wave semantics still use the unified hit-by-hit boss-survival lane.
...
49. COMPLETE: recertify the post-audit output contract and timing evidence for the all-tier Boss Waves matrix inside the current boss-contact scope. `out/boss_wave_milestone_matrix.json` is explicitly tracked in the active output contract, the tracked-output boundary test includes it, and bounded `run_stats` timing reports `boss_wave_milestone_matrix_build_ms` separately from `write_outputs_ms`. Current local evidence after the matrix performance and certification repair: full pytest passed 524 tests, `app.run_stats --include-boss-wave-milestone-matrix --boss-wave-contact-time-seconds 10 --boss-wave-orb-boss-total-damage-pct 6` completed, selected matrix rows have no model-completion blockers, and matrix build timing was about 55.9s with matrix write timing about 20.9ms and output artifact writes about 286.3ms. The model is still intentionally labeled as a partial boss-contact model rather than a full all-enemy max-wave model.
...
12. Current worktree full-suite recertification is green after the matrix performance and certification repair on 2026-05-25: `.venv\Scripts\python.exe -m pytest -q` passed 524 tests. Continue to distinguish this from a claim that every EP comparison is semantically perfect; it means the repo contract tests now match current KB/IDS/QE implementation reality.
13. Current EP compare publication has zero true formula mismatches across the expanded 52-destination EP export compare: 17 exact matches, 9 close matches, 4 ledger non-comparable rows, and 22 stage-scope rows documenting EP shortcuts or missing user-specific export inputs rather than simulator formula failures. EP `health_raw` now compares against the QE-published raw-health helper before the Armor-slot module health multiplier, while the existing post-armor health pool remains available for wall/eHP consumers. eDamage helper locals for Smart Missiles heatup, Poison Swamp Death Creep, ILM Charged Mines, and Chrono Field+ now map to QE-published helper surfaces; locked or unowned UW helper deltas remain stage-scope rather than hidden calibration.
