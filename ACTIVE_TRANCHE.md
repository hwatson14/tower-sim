...
27. NEXT: keep `boss_time_to_contact_seconds` as an explicit manual runtime input for now; continue only with targeted contract cleanup or performance/residue work from the replacement-owned product path, with no legacy executor coexistence to protect.
...
8. Boss Waves perk-mode behavior is closed locally for the replacement path: runtime timeline, max-progression static policy, and disabled perk paths now produce distinct row evolution and diagnostics.
