...
16. Remaining work for full max-wave completion is tracked in `BURNDOWN.yaml:remaining_max_wave_completion`. The current milestone matrix is still explicitly scoped as boss-contact survivability, and the user has narrowed current scope to avoid separate non-boss terminal-pressure formula work; Boss Waves is the sanctioned proxy/test surface for stacked enemy pressure for now. Payloads and matrix rows publish a normalized `model_certification` status/blocker set so partial outputs cannot appear fully certified, plus a `runtime_override_closure` map and requirement-applicability map. `MW-6`, `MW-5`, and `MW-7` are closed target-by-target for milestone/PB references, Dissonant Run mask certification, and four-policy perk/loadout acceptance. `MW-3` is closed by deriving omitted boss contact time from 2s base plus QE-owned CF/EN/Slow Aura effects while still labeling caller-supplied overrides. `MW-2` is closed for normal boss-contact matrices and remains explicit-input-only for tournament Damage/Health Decay because public sources do not provide magnitudes. `MW-1` is deferred out of current scope by user direction. `MW-4` is updated/closed for selected default matrix rows by using unified hit-by-hit semantics across eHP/GC/farming loadouts and by using the QE `derived::edamage_ep` boss-exposure replacement model for GC damage; caller-owned bridge inputs remain optional overrides, not hidden workbook calibration. `MW-8` is closed for CLI/output/full-pytest recertification in this latest boss-damage closure; in-app Browser screenshot evidence remains optional and dependent on Browser plugin availability.
