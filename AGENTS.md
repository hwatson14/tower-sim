...
## Non‑negotiables
1. **Deterministic only, except for perks**: no RNG, no sampling, no Monte Carlo.
...
4. **No silent changes**: every change must be explained in the PR description, including formulas and data sources.
5. **Tests required**: every new module must have unit tests. If a test depends on data snapshots, pin the snapshot.
6. **No drive‑by refactors**: do not reformat or rename unrelated code.
