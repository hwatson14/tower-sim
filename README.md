...
| `out/max_wave_latest.json` + `out/lineage_manifest_latest.json` | `python -m tower_sim.run.runner --spec tests/fixtures/specs/sample_spec.yaml --ids tests/fixtures/tower-sim-data/_IDS.csv` | Local run output | Canonical MAX_WAVE runner payload + lineage manifest payload. |
...
```bash
python -m tower_sim.run.runner --spec tests/fixtures/specs/sample_spec.yaml
```

Optional patch overlay and IDS input:

```bash
python -m tower_sim.run.runner --spec tests/fixtures/specs/sample_spec.yaml --patch /path/to/patch.yaml --ids tests/fixtures/tower-sim-data/_IDS.csv
```
