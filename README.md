...
| `out/max_wave_latest.json` + `out/lineage_manifest_latest.json` | `python -m tower_sim.run MAX_WAVE --spec fixtures/specs/max_wave.yaml --ids tests/fixtures/tower-sim-data/_IDS.csv` | Local run output | Canonical MAX_WAVE runner payload + lineage manifest payload. |
...
| `out/stat_lineage_report_latest.json` | `python -m tower_sim.audit.stat_lineage_report --json-out out/stat_lineage_report_latest.json` | Local run output | Deterministic burn-down summary of unresolved lineage stat/contributor wiring pairs plus required MAX_WAVE gaps. |

Stat-lineage burn-down report command (fail-closed on manifest shape keys):
...
## Canonical MAX_WAVE runner command

Use exactly one blessed invocation path for end-to-end MAX_WAVE runs:

```bash
python -m tower_sim.run MAX_WAVE --spec fixtures/specs/max_wave.yaml
```

Optional patch overlay and IDS input:

```bash
python -m tower_sim.run MAX_WAVE --spec fixtures/specs/max_wave.yaml --patch tests/fixtures/specs/v1_patch_override.yaml --ids tests/fixtures/tower-sim-data/_IDS.csv
```

This command writes runtime artifacts to:
- `out/max_wave_latest.json`
