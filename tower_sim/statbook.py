...
from tower_sim.stat_registry import Phase
...
class CanonicalStatBook:
    ...
    def _sorted_rows(self) -> Iterable[CanonicalStatRow]:
        return sorted(
            self.rows,
            key=lambda row: (row.stat_id, row.phase.value, row.provenance),
        )
