...
def test_pipeline_run_stats_query_rows_publish_qe_derived_wall_semantics(canonical_pipeline_artifacts):
    ...
    assert start_farming['state::tower.free_attack_upgrade_chance_pct']['final_value'] == pytest.approx(0.5)
    assert max_farming['state::tower.free_attack_upgrade_chance_pct']['final_value'] == pytest.approx(75.0)
    ...
    assert start_farming['support_surface::ehp.black_hole_duration_seconds']['final_value'] == pytest.approx(15.0)
    assert start_farming['support_surface::ehp.black_hole_cooldown_seconds']['final_value'] == pytest.approx(200.0)
    assert start_farming['support_surface::ehp.health_relic_pct']['final_value'] == pytest.approx(0.0)
    assert start_farming['support_surface::ehp.dabs_relic_pct']['final_value'] == pytest.approx(0.0)
    assert start_farming['support_surface::ehp.def_pct_relic_pct']['final_value'] == pytest.approx(0.0)
    assert start_farming['support_surface::eecon.adstarter_theme_relic_factor']['final_value'] == pytest.approx(1.0)
    assert start_farming['support_surface::eecon.freeup_attack_relic_pct']['final_value'] == pytest.approx(0.0)
    assert start_farming['support_surface::eecon.freeup_defense_relic_pct']['final_value'] == pytest.approx(0.0)
    assert start_farming['support_surface::eecon.freeup_utility_relic_pct']['final_value'] == pytest.approx(0.0)
    assert start_farming['state::uw.black_hole.base_duration_seconds']['final_value'] == pytest.approx(15.0)
    assert start_farming['state::uw.black_hole.base_cooldown_seconds']['final_value'] == pytest.approx(200.0)
    assert start_farming['state::uw.black_hole.duration_seconds']['final_value'] == pytest.approx(15.0)
    assert start_farming['state::uw.black_hole.cooldown_seconds']['final_value'] == pytest.approx(200.0)
    assert start_farming['state::uw.golden_tower.base_duration_seconds']['final_value'] == pytest.approx(15.0)
    assert start_farming['state::uw.golden_tower.base_cooldown_seconds']['final_value'] == pytest.approx(300.0)
    assert start_farming['state::uw.golden_tower.duration_seconds']['final_value'] == pytest.approx(15.0)
    assert start_farming['state::uw.golden_tower.cooldown_seconds']['final_value'] == pytest.approx(300.0)
    assert start_farming['derived::ehp.primordial_black_hole_uptime']['final_value'] == pytest.approx(15.0 / 215.0)
    assert start_farming['derived::ehp.primordial_black_hole_damage_reduction_factor']['final_value'] == pytest.approx(1.0)
    assert start_farming['derived::ehp.health_relic_factor']['final_value'] == pytest.approx(1.0)
    assert start_farming['derived::ehp.dabs_relic_factor']['final_value'] == pytest.approx(1.0)
    assert start_farming['derived::ehp.def_pct_relic_term']['final_value'] == pytest.approx(0.0)
    assert start_farming['derived::eecon.base_meta_factor']['final_value'] == pytest.approx(1.0)
