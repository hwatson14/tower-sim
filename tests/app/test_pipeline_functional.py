...
def test_execute_pipeline_smoke_and_trace_contract(tmp_path):
    ...
    assert any("ep_oracle_compare.json" in f for f in written)
    assert any("line_by_line_verification.json" in f for f in written)
...
def test_sharded_evaluators_parity(tmp_path):
    ...
    # Check for a known destination if possible, or just non-empty
