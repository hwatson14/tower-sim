...
def test_live_boss_waves_product_seam_exposes_shadow_validation_subset_read_only():
    ...
    assert download_rows == rows, "Phase 2B export rows must use the selected replacement rows"
    assert legacy_shadow_rows, "Phase 2B must keep legacy rows available for shadow/rollback validation"
    first = rows[0]
    for field in (
        "display_wave",
        "attack_wave",
    ...
    assert first.get("replacement_source") == "phase2b_replacement"
