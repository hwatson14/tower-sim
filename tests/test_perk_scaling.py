...
def test_free_upgrades_enhancement_applies_to_all_three_free_upgrade_stats(tmp_path):
    ...
    assert attack == pytest.approx((49.5 + 6.0) * 1.12)
    assert defense == pytest.approx((49.5 + 3.0) * 1.12)
    assert utility == pytest.approx((49.5 + 4.0 + 10.0 + 0.8) * 1.12)
...
def test_free_upgrades_card_is_split_into_canonical_free_upgrade_stats_and_values_match_ep_baseline():
    ...
    assert statbook.rows["canonical_stat::free_attack_upgrade_chance_pct"].final_value == pytest.approx(107.7384, rel=1e-6)
    assert statbook.rows["canonical_stat::free_defense_upgrade_chance_pct"].final_value == pytest.approx(104.6892, rel=1e-6)
    assert statbook.rows["canonical_stat::free_utility_upgrade_chance_pct"].final_value == pytest.approx(118.28544, rel=1e-6)
