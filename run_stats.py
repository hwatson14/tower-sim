...
def _build_generated_max_progression_perk_config(ids_raw, primary_config: dict | None = None, *, default_policy_path: Path = _PERK_MAX_POLICY_PATH) -> tuple[dict, dict]:
    from tower_sim.engines.perk_timeline_generator import generate_timeline, perk_state_at_wave
    ...
    generated = {
        'perk_presets': {
            'ProjectedMax_GeneratedTimeline': selections,
        },
        'active_perk_preset': 'ProjectedMax_GeneratedTimeline',
...
def _classify_compare_status(destination: str, contract: dict, package_row, ep_entry: dict, package_stage_context: dict | None = None):
    compare_policy = contract.get('compare_policy', 'normal')
    stage_context = _ep_stage_context_for_destination(destination, package_stage_context or {})
    ...
    pkg_cmp, ep_cmp, notes = _normalize_compare_values(destination, compare_policy, package_value, ep_value)
    delta = None
    rel_pct = None
    try:
...
def _annotate_compare_display_fields(ep_compare: dict) -> None:
...
def _ensure_compare_authoritative_verdict_fields(compare: dict) -> dict:
    for payload in (compare or {}).values():
        ...
        if kb_alignment_status is None:
            kb_alignment_status = _kb_alignment_status_from_compare_status(status)
...
def _ensure_line_verification_authoritative_verdict_fields(verification: dict) -> dict:
    for payload in (verification or {}).values():
        ...
        kb_alignment_status = payload.get('kb_alignment_status')
        if kb_alignment_status is None:
            kb_alignment_status = _kb_alignment_status_from_compare_status(compare_status)
            payload['kb_alignment_status'] = kb_alignment_status
        if payload.get('verdict') is None:
            payload['verdict'] = _verdict_from_verification(payload.get('verification_status'), compare_status)
    return verification
...
def _build_survivor_closure_report(ep_compare: dict, line_verification: dict) -> dict:
    tracked = [
...
def _build_ep_compare(ep_oracle, statbook_rows_by_preset, formula_ledger, package_stage_context):
    compare = {}
    for dest, ep in ep_oracle.items():
        stage_context = _ep_stage_context_for_destination(dest, package_stage_context)
        ...
        compare_state_key = _compare_state_key_for_destination(dest, package_stage_context.get('default_compare_preset', 'Farming') if package_stage_context else 'Farming')
        ...
        raw_pkg_snapshot = _contributor_snapshot(raw_pkg)
        pkg, assumption_notes = _apply_projected_runtime_compare_assumptions(dest, raw_pkg, stage_context)
        pkg_snapshot = _contributor_snapshot(pkg)
        contract = _formula_contract(formula_ledger, dest)
        status, delta, rel_pct, notes = _classify_compare_status(dest, contract, pkg, ep, package_stage_context)
...
def _kb_alignment_status_from_compare_status(compare_status: str | None) -> str:
...
def _verdict_from_verification(verification_status: str, compare_status: str | None) -> str:
...
def _build_compare_status_summary(ep_compare: dict) -> dict:
...
def _build_survivability_residue_analysis(ep_compare: dict, compare_situation_fit_matrix: dict, statbook_dict: dict) -> dict:
    destinations = [
        'canonical_stat::tower_hp',
        'canonical_stat::tower_regen',
        'canonical_stat::tower_defense_absolute',
...
def _build_compare_situation_fit_matrix(ids_raw, loadout_config, perk_config, formula_ledger, ep_oracle: dict) -> dict:
    ...
    for state_key, preset, perk_state, forced_preset_perk_states in states:
        ...
        compare = _build_ep_compare(ep_oracle, rows_by_preset, formula_ledger, stage_context)
        views[state_key] = {
            'preset': preset,
            'perk_state': perk_state,
...
def _build_line_by_line_verification(statbook_dict, ep_compare, formula_ledger):
    verification = {}
    ...
    for key, row in rows.items():
        ...
        contract = _formula_contract(formula_ledger, key)
        consumed_contributors = None
        notes_text = str(row.get('notes') or '')
        import re
        m = re.search(r'Consumed\s+(\d+)\/(\d+)\s+contributors', notes_text)
        if m:
            consumed_contributors = {'consumed': int(m.group(1)), 'listed': int(m.group(2))}
        ...
        kb_alignment_status = _kb_alignment_status_from_compare_status(compare_status)
        verdict = _verdict_from_verification(verification_status, compare_status)
        verification[key] = {
            'destination': key,
            'status': row.get('status'),
...
def main() -> int:
    ...
    ep_compare = _build_ep_compare(ep_oracle, compare_rows_by_preset, formula_ledger, package_stage_context)
    ep_compare_publishable = _build_ep_compare(ep_oracle, compare_publishable_rows_by_preset, formula_ledger, package_stage_context)
    ...
    projected_ep_compare_publishable = _build_ep_compare(ep_oracle, projected_compare_publishable_rows_by_preset, formula_ledger, projected_stage_context)
    ...
    line_verification = _build_line_by_line_verification(statbook_publishable_dict, ep_compare_publishable, formula_ledger)
    ...
    # Compatibility mirrors for consumers that still read legacy top-level FINAL_* artifacts.
    legacy_cols = [
        'destination', 'status', 'verification_status', 'kb_alignment_status',
        'formula_class', 'resolver', 'contributor_count', 'ep_compare_status',
        'ep_relative_delta_pct', 'final_value', 'display_value', 'notes',
    ]
    legacy_df = verification_df.copy()
    legacy_df = legacy_df.rename(columns={'contributor_count': 'source_count'})
    for col in legacy_cols:
        if col not in legacy_df.columns:
            legacy_df[col] = None
    legacy_df = legacy_df[legacy_cols].rename(columns={'destination': 'destination_id'})
    legacy_df.to_csv(Path.cwd() / 'FINAL_ALL_CALCULATED_STATS.csv', index=False)
    (Path.cwd() / 'FINAL_ALL_CALCULATED_STATS.json').write_text(legacy_df.to_json(orient='records', indent=2))
    return 0
