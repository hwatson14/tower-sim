...
- [x] Introduce canonical MAX_WAVE runner spine with module entrypoint `python -m tower_sim.run --spec fixtures/specs/max_wave.yaml`; `tower_sim/run/runner.py` stays a thin delegator and writes `out/max_wave_latest.json` plus `out/lineage_manifest_latest.json`.
