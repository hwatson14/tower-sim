...
## Status posture after Phase 2E evidence capture

- `pass` is used only where bounded Phase 2E evidence is now recorded for the relevant family surface.
- `in_progress` marks families that have some query-family parity evidence but still lack compatibility-entrypoint parity closure.
- `blocked` marks benchmark or parity work that cannot proceed until the compatibility entrypoint delegates that family explicitly.
- `not_required_yet` remains reserved for undelegated rows so the manifest does not imply benchmark obligations that do not apply yet.
