"""
Greedy path ranker — generates optimal upgrade sequences per EP's methodology.

For each spend domain (labs), evaluates every candidate +1 upgrade, computes
ROI = score_delta / cost, picks the max-ROI candidate, and repeats.

eHP lab paths match EP 100% on modelable labs (module labs are not modeled).
eDamage/eEcon paths use simplified composites — see scorer.py for accuracy notes.
"""
from __future__ import annotations
from typing import Callable
from optimizer.scorer import compute_ehp, compute_edamage, compute_eecon
