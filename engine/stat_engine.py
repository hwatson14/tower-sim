...
def _apply_free_upgrade_chance_formula_from_routed_contributors(rows: Dict[str, StatRow]) -> None:
    ...
    for key in [
        ...
        if stat_row is None:
            continue
        additive_base = 0.0
        post_multiplier = support_multiplier
        for contributor in stat_row.contributors:
            ...
            if row.source_family in {'relic', 'vault'}:
                post_multiplier *= (1.0 + v) if 0.0 <= v <= 1.0 else v
            else:
                additive_base += v
        stat_row.final_value = additive_base * post_multiplier
        stat_row.status = 'resolved'
        stat_row.notes = 'Free-upgrade formula from routed contributors: (workshop + card + perk + module-substat additive bonuses) x free-upgrade support multiplier x relic/vault multipliers.'
