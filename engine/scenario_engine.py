...
Takes stat engine outputs + scenario configuration, emits scenario-owned
fixed-for-run surfaces needed by timing, progression, and analysis layers.
...
class ScenarioSurfaces:
    ...
    # and uptime-weighted average effects now belong to engine/timing_engine.py.
...
def compute_scenario_surfaces(config: ScenarioConfig) -> ScenarioSurfaces:
    ...
    # Timing-owned surfaces are emitted by engine/timing_engine.py.
