...
def _ep_shock_factor(rows: Dict[str, StatRow]) -> float:
    ...
    cl_active = _bool(
        rows,
        [_runtime('uw.chain_lightning.active'), _mech('uw.chain_lightning.active')],
        default=_get_first(rows, [_mech('uw.chain_lightning.damage_multiplier'), _runtime('uw.chain_lightning.damage_multiplier')], 0.0) > 0.0,
    )
    shock_enabled = _bool(rows, [_runtime('shock.enabled'), _mech('shock.enabled')], default=cl_active)
    if not (cl_active and shock_enabled):
        return 1.0
    shock_level = _get_first(rows, ['support_surface::shock.level', _runtime('shock.level'), _mech('shock.level')], 0.0)
    has_dc = _get_first(rows, [_mech('module.dimension_core.multiplier'), _mech('module.dimension_core.damage_multiplier')], 0.0) > 0.0
    dc_mult = _get_first(rows, [_mech('module.dimension_core.multiplier'), _mech('module.dimension_core.damage_multiplier')], 0.0)
...
def _ep_tradeoff_defense_factor(rows: Dict[str, StatRow]) -> float:
    return _get_first(
        ...
        [
            ...
            _runtime('perk.tradeoff_defense_factor'),
            _mech('perk.tradeoff_defense_factor'),
            'support_surface::perk.improve_tradeoff_defense_factor',
            _runtime('perk.improve_tradeoff_defense_factor'),
            _mech('perk.improve_tradeoff_defense_factor'),
        ],
        1.0,
    )
...
def _ep_armor_factor(rows: Dict[str, StatRow]) -> float:
    direct = _get_first(
        ...
        [
            ...
            _runtime('armor.multiplier'),
            _mech('armor.multiplier'),
            _canon('tower_armor_multiplier'),
        ],
        0.0,
    )
    if direct > 0.0:
    ...
    primary_bonus = _get_first(rows, [
        ...
        _mech('module.armor.primary_bonus'),
        _mech('module.armor.primary_multiplier'),
        _mech('module.armor.multiplier'),
    ], 1.0)
    has_assist = _bool(rows, [
        'support_surface::ehp.armor_assist_enabled',
        _mech('module.armor.assist_enabled'),
        _mech('module.armor.has_assist'),
    ], False)
    assist_bonus = _get_first(rows, [
        'support_surface::ehp.armor_assist_bonus',
        _mech('module.armor.assist_bonus_multiplier'),
        _mech('module.armor.assist_bonus'),
    ], 1.0)
    stone_bac = _get_first(rows, [
        'support_surface::ehp.armor_assist_stone_bonus_pct',
        _mech('module.armor.assist_stone_bonus_pct'),
    ], 0.0)
    lab_bac = _get_first(rows, [
        'support_surface::ehp.armor_assist_lab_bonus_pct',
        _mech('module.armor.assist_lab_bonus_pct'),
    ], 0.0)
...
def _ep_chain_thunder_factor(rows: Dict[str, StatRow]) -> float:
    reduction_pct = _get_first(
        ...
        [
            ...
            _runtime('uw.chain_thunder.reduction_pct'),
            _mech('uw.chain_thunder.reduction_pct'),
        ],
        0.0,
    )
...
def publish_derived_composites(rows: Dict[str, StatRow]) -> None:
    ...
    health_factor = _get_first(rows, ['support_surface::ehp.health_factor', _runtime('ehp.health_factor')], 0.0)
    health_ws = _get_first(rows, [_canon('tower_hp'), 'support_surface::ehp.health_ws'], 0.0)
    ...
    armor_primary_factor = _get_first(rows, [
        ...
        'support_surface::ehp.armor_primary_bonus',
        _mech('module.armor.primary_factor'),
        _mech('module.armor.primary_bonus'),
        _mech('module.armor.primary_multiplier'),
        _mech('module.armor.multiplier'),
    ], 1.0)
    ...
    recovery_ws = _get_first(rows, [_canon('max_recovery_multiplier'), 'support_surface::ehp.max_recovery_ws'], 0.0)
    ...
    dabs_ws = _get_first(rows, [_canon('tower_defense_absolute'), 'support_surface::ehp.dabs_ws'], 0.0)
    ...
    def_pct_ws = _get_first(rows, [_canon('tower_defense_pct'), 'support_surface::ehp.def_pct_ws'], 0.0)
    ...
    if chrono_duration < 0.0:
        chrono_duration = _tp(rows, _mech('uw.chrono_field.duration_seconds'), _runtime('uw.chrono_field.duration_seconds'))
    chrono_cooldown = _get_first(rows, ['support_surface::ehp.chrono_field_cooldown_seconds', _mech('uw.chrono_field.cooldown_seconds')], 0.0)
    chrono_reduction_pct = _get_first(rows, ['support_surface::ehp.chrono_field_damage_reduction_pct', _mech('uw.chrono_field.damage_reduction_pct')], 0.0)
    ...
    chain_thunder_reduction_pct = _get_first(rows, ['support_surface::ehp.chain_thunder_reduction_pct', _runtime('uw.chain_thunder.reduction_pct'), _mech('uw.chain_thunder.reduction_pct')], 0.0)
    ...
    ehp_contrib = [_contributor(rows, _canon('tower_hp'), 'ehp'), _contributor(rows, _canon('wall_hp'), 'ehp')]
    ...
    _publish(rows, 'derived::ehp.dabs_factor', dabs, 'scalar', [_contributor(rows, _canon('tower_defense_absolute'), 'ehp.dabs_factor')], 'EPH_DABS-like additive absolute defense term [EP helper support surface]')
    ...
    _publish(rows, 'derived::ehp.defense_taken_factor', defense_taken_factor, 'multiplier', [_contributor(rows, _canon('tower_defense_pct'), 'ehp.defense_taken_factor')], '1/(1-EPH_DEF_PCT) [EP helper support surface]')
    ...
    _publish(rows, 'derived::ehp.chrono_field_uptime', cfu, 'ratio', [_contributor(rows, _mech('uw.chrono_field.duration_seconds'), 'ehp.chrono_field_uptime')], 'chrono field uptime [EP helper support surface]')
    _publish(rows, 'derived::ehp.chrono_field_damage_reduction_factor', cf_factor, 'multiplier', [_contributor(rows, _mech('uw.chrono_field.damage_reduction_pct'), 'ehp.chrono_field_damage_reduction_factor')], 'chrono field reduction factor [EP helper support surface]')
    ...
    _publish(rows, 'derived::ehp.chain_thunder_factor', chain_thunder_factor, 'multiplier', [_contributor(rows, _mech('uw.chain_thunder.reduction_pct'), 'ehp.chain_thunder_factor')], 'EP CN5 chain thunder factor [EP helper support surface]')
    ...
    _publish(rows, 'derived::ehp.tradeoff_defense_factor', tradeoff_defense_factor, 'multiplier', [_contributor(rows, _mech('perk.tradeoff_defense_factor'), 'ehp.tradeoff_defense_factor')], 'improve trade-off defense factor [EP helper support surface]')
    ...
    _publish(rows, 'derived::ehp_ep_helper.cg_dabs_factor', dabs_effective, 'scalar', [_contributor(rows, _canon('tower_defense_absolute'), 'ehp.ep_cg_dabs_factor')], 'EP eHP CG5 DABS factor alias [EP helper support surface]')
    _publish(rows, 'derived::ehp_ep_helper.ch_defense_taken_factor', defense_taken_factor, 'multiplier', [_contributor(rows, _canon('tower_defense_pct'), 'ehp.ep_ch_defense_taken_factor')], 'EP eHP CH5 defense taken factor alias [EP helper support surface]')
    _publish(rows, 'derived::ehp_ep_helper.ck_tradeoff_factor', tradeoff_defense_factor, 'multiplier', [_contributor(rows, _mech('perk.tradeoff_defense_factor'), 'ehp.ep_ck_tradeoff_factor')], 'EP eHP CK5 tradeoff factor alias [EP helper support surface]')
    _publish(rows, 'derived::ehp_ep_helper.cl_cfdr_factor', cf_factor, 'multiplier', [_contributor(rows, _mech('uw.chrono_field.damage_reduction_pct'), 'ehp.ep_cl_cfdr_factor')], 'EP eHP CL5 chrono damage reduction factor alias [EP helper support surface]')
    _publish(rows, 'derived::ehp_ep_helper.cm_chain_thunder_factor', chain_thunder_factor, 'multiplier', [_contributor(rows, _mech('uw.chain_thunder.reduction_pct'), 'ehp.ep_cm_chain_thunder_factor')], 'EP eHP CM5 chain thunder factor alias [EP helper support surface]')
    ...
    td = _get(rows, _canon('tower_damage'))
    asp = _get_first(rows, [_canon('tower_attack_speed'), 'derived::attack_speed'])
    dpm = _get(rows, _canon('tower_damage_per_meter_multiplier'))
    rng = _get(rows, _canon('tower_range_m'))
    kill_at_range = _get_first(rows, ['support_surface::timing.kill_at_range_multiplier', _runtime('targeting.kill_at_range_multiplier'), _mech('targeting.kill_at_range_multiplier')], 1.0)

    perk_damage_factor = _get_first(rows, ['support_surface::perk.damage_multiplier', _runtime('perk.damage_multiplier'), _mech('perk.damage_multiplier')], 1.0)
    tradeoff_damage_factor = _get_first(rows, ['support_surface::perk.tradeoff_damage_multiplier', _runtime('perk.tradeoff_damage_multiplier'), _mech('perk.tradeoff_damage_multiplier')], 1.0)
    shock_factor = _ep_shock_factor(rows)
    card_damage_mastery_factor = _get_first(rows, [_runtime('cards.damage.mastery_multiplier'), _mech('cards.damage.mastery_multiplier')], 1.0)

    cannon_module_factor = _get_first(rows, [_mech('module.cannon.primary_damage_multiplier'), _mech('module.cannon.damage_multiplier'), _mech('module.cannon.multiplier')], 1.0)
    cannon_assist_enabled = _bool(rows, [_mech('module.cannon.assist_enabled'), _mech('module.cannon.has_assist')], False)
    cannon_assist_bonus = _get_first(rows, [_mech('module.cannon.assist_bonus_multiplier'), _mech('module.cannon.assist_multiplier')], 1.0)
    cannon_assist_factor = _ep_module_bonus(
        cannon_module_factor,
        cannon_assist_enabled,
        cannon_assist_bonus,
        _get_first(rows, [_mech('module.cannon.assist_stone_bonus_pct')], 0.0),
        _get_first(rows, [_mech('module.cannon.assist_lab_bonus_pct')], 0.0),
    )

    acp_raw = _get_first(rows, [_mech('module.anti_cube_portal.damage_multiplier'), _mech('module.anti_cube_portal.multiplier'), _mech('module.acp.damage_multiplier'), _mech('module.acp.multiplier')], 0.0)
    acp_factor = _ep_shockwave_damage(
        acp_raw,
        _get_first(rows, [_canon('shockwave_size_level'), _mech('shockwave.size_ws_level')], 0.0),
        _get_first(rows, [_runtime('shockwave.size_lab_level'), _mech('shockwave.size_lab_level')], 0.0),
        _get_first(rows, [_canon('shockwave_frequency_level'), _mech('shockwave.frequency_ws_level')], 0.0),
        _get_first(rows, [_runtime('shockwave.frequency_vault_bonus_seconds'), _mech('shockwave.frequency_vault_bonus_seconds')], 0.0),
        _get_first(rows, [_mech('shockwave.frequency_substat_seconds'), _runtime('shockwave.frequency_substat_seconds')], 0.0),
    )
    amp_strike_factor = _get_first(rows, ['support_surface::amp_strike.damage_multiplier', _mech('amp_strike.damage_multiplier')], 1.0)
    ...
    cc = _get(rows, _canon('tower_crit_chance_pct'))
    cf = _get(rows, _canon('tower_crit_multiplier'))
    scc = _get(rows, _canon('tower_supercrit_chance_pct'))
    scm = _get(rows, _canon('tower_supercrit_multiplier'))
    being_annihilator = _get_first(rows, [_mech('module.being_annihilator.streak_count'), _runtime('module.being_annihilator.streak_count')], 0.0)
    ...
    multishot_factor = _ep_multishot(_get(rows, _canon('tower_multishot_chance_pct')), _get(rows, _canon('tower_multishot_targets')))
    bounce_factor = _ep_bounceshot(
        _get(rows, _canon('tower_bounce_shot_chance_pct')),
        _get(rows, _canon('tower_bounce_shot_targets')),
        _get_first(rows, [_mech('module.astral_deliverance.bounce_bonus_per_hop'), _mech('module.astral_deliverance.multiplier_per_bounce'), _mech('module.astral_deliverance.multiplier')], 0.0),
    )
    rapidfire_factor = _ep_rapidfire_factor(_get(rows, _canon('tower_rapid_fire_chance_pct')), _get(rows, _canon('tower_rapid_fire_duration_seconds')), bps)
    range_dpm_factor = _ep_rangedpm(rng, dpm, kill_at_range)

    sl_quantity = _get_first(rows, [_mech('uw.spotlight.quantity'), _runtime('uw.spotlight.quantity')], 0.0)
    sl_angle = _get_first(rows, [_mech('uw.spotlight.angle_degrees'), _runtime('uw.spotlight.angle_degrees'), _canon('uw.spotlight.angle_degrees')], 0.0)
    sl_light_range = _get_first(rows, [_mech('uw.spotlight.light_range_bonus'), _runtime('uw.spotlight.light_range_bonus')], 0.0)
    spotlight_light_range_factor = _stat_uw_sl_final_lr(sl_light_range, range_dpm_factor)
    sl_bonus = _stat_uw_sl_final_dmg(
        _get_first(rows, [_mech('uw.spotlight.damage_multiplier'), _runtime('uw.spotlight.damage_multiplier')], 1.0),
        spotlight_light_range_factor,
        _get_first(rows, [_mech('uw.spotlight.damage_substat'), _runtime('uw.spotlight.damage_substat')], 0.0),
        _get_first(rows, [_mech('uw.spotlight.relic_bonus')], 0.0),
        _get_first(rows, [_mech('uw.spotlight.vault_bonus')], 0.0),
        _bool(rows, [_runtime('perk.spotlight_damage.active')], False),
    )
    ...
    super_tower_factor = _ep_super_tower_effective_bonus(
        _bool(rows, [_runtime('cards.super_tower.active'), _mech('cards.super_tower.active')], False),
        _bool(rows, [_runtime('cards.super_tower.mastery_active'), _mech('cards.super_tower.mastery_active')], False),
        _get_first(rows, [_runtime('cards.super_tower.bonus_multiplier'), _mech('cards.super_tower.bonus_multiplier')], 1.0),
        _get_first(rows, [_runtime('cards.super_tower.cooldown_seconds'), _mech('cards.super_tower.cooldown_seconds')], 1.0),
        sl_quantity > 0 and sl_angle > 0,
        sl_quantity,
        sl_angle,
    )
    rend_factor = _ep_maxrend(
        _bool(rows, [_runtime('rend.active'), _mech('rend.active')], False),
        _get_first(rows, [_runtime('rend.lab_level'), _mech('rend.lab_level')], 0.0),
        _get_first(rows, [_runtime('rend.substat'), _mech('rend.substat')], 0.0),
        _get_first(rows, [_runtime('rend.enhancement_level'), _mech('rend.enhancement_level')], 0.0),
    )
    ...
    has_aoe_card = _bool(rows, [_runtime('cards.aoe.active'), _mech('cards.aoe.active')], False)
    aoe_card_level = _get_first(rows, [_runtime('cards.aoe.level'), _mech('cards.aoe.level')], 0.0)

    dw = _ep_dw_dps(
        _bool(rows, [_runtime('uw.death_wave.active'), _mech('uw.death_wave.active')], default=_get_first(rows, [_mech('uw.death_wave.damage_multiplier'), _runtime('uw.death_wave.damage_multiplier')], 0.0) > 0.0),
        _get_first(rows, ['support_surface::uw.death_wave.final_damage', _mech('uw.death_wave.damage_multiplier'), _runtime('uw.death_wave.damage_multiplier')], 0.0),
        _get_first(rows, ['support_surface::uw.death_wave.final_quantity', _mech('uw.death_wave.quantity'), _runtime('uw.death_wave.quantity')], 0.0),
        _get_first(rows, ['support_surface::uw.death_wave.final_cooldown_seconds', _mech('uw.death_wave.cooldown_seconds'), _runtime('uw.death_wave.cooldown_seconds')], 1.0),
        _get_first(rows, ['support_surface::uw.death_wave.damage_amp', _mech('uw.death_wave.damage_amp'), _runtime('uw.death_wave.damage_amp')], 0.0),
    )
    cl = _ep_cl_dps(
        _get_first(rows, ['support_surface::uw.chain_lightning.final_damage', _mech('uw.chain_lightning.damage_multiplier'), _runtime('uw.chain_lightning.damage_multiplier')], 0.0),
        _get_first(rows, ['support_surface::uw.chain_lightning.final_quantity', _mech('uw.chain_lightning.quantity'), _runtime('uw.chain_lightning.quantity')], 0.0),
        _get_first(rows, ['support_surface::uw.chain_lightning.final_chance', _mech('uw.chain_lightning.chance_pct'), _runtime('uw.chain_lightning.chance_pct')], 0.0),
        multi_rapid_bounce,
        bps,
    )

    sm_damage = _get_first(rows, ['support_surface::uw.smart_missiles.final_damage', _mech('uw.smart_missiles.damage_multiplier'), _runtime('uw.smart_missiles.damage_multiplier')], 0.0)
    sm_quantity = _get_first(rows, ['support_surface::uw.smart_missiles.final_quantity', _mech('uw.smart_missiles.quantity'), _runtime('uw.smart_missiles.quantity')], 0.0)
    sm_cooldown = _get_first(rows, ['support_surface::uw.smart_missiles.final_cooldown_seconds', _mech('uw.smart_missiles.cooldown_seconds'), _runtime('uw.smart_missiles.cooldown_seconds')], 1.0)
    sm_coverfire_time = _get_first(rows, ['support_surface::uw.smart_missiles.cover_fire_time_seconds', _mech('uw.smart_missiles.cover_fire_time_seconds'), _runtime('uw.smart_missiles.cover_fire_time_seconds')], 0.0)
    ...
    sm_heat = _get_first(rows, ['support_surface::uw.smart_missiles.heat_multiplier', _mech('uw.smart_missiles.heat_multiplier'), _runtime('uw.smart_missiles.heat_multiplier')], 1.0)
    sm_aoe = _get_first(rows, ['support_surface::uw.smart_missiles.aoe_multiplier', _mech('uw.smart_missiles.aoe_multiplier'), _runtime('uw.smart_missiles.aoe_multiplier')], 1.0)
    ...
    slm = _ep_slm_dps(
        ...
        has_aoe_card,
        aoe_card_level,
    ) if _bool(rows, [_runtime('uw.spotlight.active'), _mech('uw.spotlight.active')], default=sl_quantity > 0.0 and sl_angle > 0.0) and _bool(rows, [_runtime('uw.smart_missiles.active'), _mech('uw.smart_missiles.active')], default=sm_damage > 0.0) else 0.0

    ps_damage = _get_first(rows, ['support_surface::uw.poison_swamp.damage', _mech('uw.poison_swamp.damage_multiplier'), _runtime('uw.poison_swamp.damage_multiplier')], 0.0)
    ps_duration = _get_first(rows, ['support_surface::uw.poison_swamp.duration_seconds', _mech('uw.poison_swamp.duration_seconds'), _runtime('uw.poison_swamp.duration_seconds')], 0.0)
    ps_cooldown = _get_first(rows, ['support_surface::uw.poison_swamp.cooldown_seconds', _mech('uw.poison_swamp.cooldown_seconds'), _runtime('uw.poison_swamp.cooldown_seconds')], 1.0)
    ps_deathcreep = _get_first(rows, ['support_surface::uw.poison_swamp.death_creep_multiplier', _mech('uw.poison_swamp.death_creep_multiplier'), _runtime('uw.poison_swamp.death_creep_multiplier')], 1.0)
    ps_rend = _get_first(rows, ['support_surface::uw.poison_swamp.rend_multiplier'], 0.0)
    if ps_rend <= 0.0:
        ps_rend = 1.0 + max(0.0, rend_factor) * _get_first(rows, ['support_surface::uw.poison_swamp.rend_scalar', _mech('uw.poison_swamp.rend_scalar')], 0.0)
    ps = _ep_ps_dps(
        _bool(rows, [_runtime('uw.poison_swamp.active'), _mech('uw.poison_swamp.active')], default=ps_damage > 0.0),
        ps_damage,
        ps_duration,
        ps_cooldown,
        ps_deathcreep,
    ...
    ilm_quantity = _get_first(rows, ['support_surface::uw.inner_land_mines.final_quantity', _mech('uw.inner_land_mines.quantity'), _runtime('uw.inner_land_mines.quantity')], 0.0)
    ilm_cooldown = _get_first(rows, ['support_surface::uw.inner_land_mines.cooldown_seconds', _mech('uw.inner_land_mines.cooldown_seconds'), _runtime('uw.inner_land_mines.cooldown_seconds')], 1.0)
    ...
    if sd_chance <= 0.0:
        sd_chance = _get_first(rows, [_runtime('land_mine.spawn_per_second'), _mech('land_mine.spawn_per_second')], 0.0) * _get_first(rows, [_runtime('land_mine.chance'), _mech('land_mine.chance')], 0.0) * _get_first(rows, [_mech('module.space_displacer.spawn_multiplier'), _mech('module.space_displacer.multiplier')], 0.0)
    chrono_jump = _get_first(rows, ['support_surface::uw.inner_land_mines.chrono_jump_multiplier', _runtime('uw.chrono_jump.multiplier'), _mech('uw.chrono_jump.multiplier')], 0.0) * _get_first(rows, ['support_surface::uw.inner_land_mines.chrono_jump_scalar'], 5.0)
    ilm = _ep_ilm_dps(
        _bool(rows, [_runtime('uw.inner_land_mines.active'), _mech('uw.inner_land_mines.active')], default=False),
        _get_first(rows, ['support_surface::uw.inner_land_mines.final_damage', _mech('uw.inner_land_mines.damage_multiplier'), _runtime('uw.inner_land_mines.damage_multiplier')], 0.0),
        ilm_spawn_per_second + sd_chance,
        chrono_jump,
        _get_first(rows, ['support_surface::uw.inner_land_mines.charged_mines', _mech('uw.inner_land_mines.charged_mines'), _runtime('uw.inner_land_mines.charged_mines')], 0.0),
        _get_first(rows, ['support_surface::uw.inner_land_mines.aoe_multiplier', _mech('uw.inner_land_mines.aoe_multiplier'), _runtime('uw.inner_land_mines.aoe_multiplier')], 1.0),
        has_aoe_card,
        aoe_card_level,
    )
    uw_damage_boost = _get_first(rows, ['support_surface::uw.total_damage_boost_multiplier', _mech('uw.damage_boost_multiplier')], 1.0)
    st_uw_mastery = _get_first(
        rows,
        [
            'support_surface::uw.super_tower_effective_uw_bonus',
            _runtime('cards.super_tower.uw_mastery_multiplier'),
            _mech('cards.super_tower.uw_mastery_multiplier'),
        ],
        1.0,
    )
    uw_crit_card = _get_first(rows, [_runtime('cards.ultimate_weapon_crit.multiplier'), _mech('cards.ultimate_weapon_crit.multiplier')], 1.0)
    ...
    if slow_factor <= 0.0:
        cf_slow_pct = _get_first(rows, [_mech('uw.chrono_field.slow_pct'), _runtime('uw.chrono_field.slow_pct')], 0.0)
        slow_factor = 1.0
        if cf_slow_pct > 0.0:
            slow_factor *= 1.0 / max(1.0 - min(0.90, _pct(cf_slow_pct)), 0.1)
    slow_factor *= _get_first(rows, ['support_surface::chrono_field.plus_exposure_multiplier', _mech('uw.chrono_field.plus_exposure_multiplier')], 1.0)
    ...
    _publish(rows, 'derived::edamage.base_damage_stack', base_damage_stack, 'scalar', [_contributor(rows, _canon('tower_damage'), 'edamage.base_damage_stack')], 'EP EM5 DC5-like base damage stack [EP helper support surface]')
    _publish(rows, 'derived::edamage.acp_factor', acp_factor, 'multiplier', [_contributor(rows, _mech('module.anti_cube_portal.damage_multiplier'), 'edamage.acp_factor')], 'EP EPD_SHOCKWAVE_DAMAGE-aligned ACP factor [EP helper support surface]')
    _publish(rows, 'derived::edamage.bullet_crit_factor', bullet_crit_factor, 'multiplier', [_contributor(rows, _canon('tower_crit_chance_pct'), 'edamage.bullet_crit_factor'), _contributor(rows, _canon('tower_supercrit_chance_pct'), 'edamage.bullet_crit_factor')], 'EPD_CRITICAL-aligned bullet crit factor [EP helper support surface]')
    _publish(rows, 'derived::edamage.bullet_per_second', bps, 'rate', [_contributor(rows, _canon('tower_attack_speed'), 'edamage.bullet_per_second')], 'EP BULLET_PER_SECOND from mechanics lineage [EP helper support surface]')
    _publish(rows, 'derived::edamage.multishot_factor', multishot_factor, 'multiplier', [_contributor(rows, _canon('tower_multishot_chance_pct'), 'edamage.multishot_factor'), _contributor(rows, _canon('tower_multishot_targets'), 'edamage.multishot_factor')], 'EPD_MULTISHOT-aligned multishot factor [EP helper support surface]')
    _publish(rows, 'derived::edamage.bounce_factor', bounce_factor, 'multiplier', [_contributor(rows, _canon('tower_bounce_shot_chance_pct'), 'edamage.bounce_factor'), _contributor(rows, _canon('tower_bounce_shot_targets'), 'edamage.bounce_factor')], 'EPD_BOUNCESHOT-aligned bounce factor [EP helper support surface]')
    _publish(rows, 'derived::edamage.rapidfire_factor', rapidfire_factor, 'multiplier', [_contributor(rows, _canon('tower_rapid_fire_chance_pct'), 'edamage.rapidfire_factor'), _contributor(rows, _canon('tower_rapid_fire_duration_seconds'), 'edamage.rapidfire_factor')], 'EPD_RAPIDFIRE-aligned rapidfire factor [EP helper support surface]')
    _publish(rows, 'derived::edamage.range_dpm_factor', range_dpm_factor, 'multiplier', [_contributor(rows, _canon('tower_damage_per_meter_multiplier'), 'edamage.range_dpm_factor'), _contributor(rows, _canon('tower_range_m'), 'edamage.range_dpm_factor')], 'EPD_RANGEDPM-aligned range x dpm x kill-at-range factor [EP helper support surface]')
    _publish(rows, 'derived::edamage.spotlight_light_range_factor', spotlight_light_range_factor, 'multiplier', [_contributor(rows, _mech('uw.spotlight.light_range'), 'edamage.spotlight_light_range_factor')], 'EP spotlight light-range factor before coverage/final bonus [EP helper support surface]')
    _publish(rows, 'derived::edamage.spotlight_bonus_term', sl_bonus, 'multiplier', [_contributor(rows, _mech('uw.spotlight.damage_multiplier'), 'edamage.spotlight_bonus_term')], 'EP spotlight bonus term before coverage [EP helper support surface]')
    _publish(rows, 'derived::edamage.spotlight_coverage', spotlight_coverage, 'ratio', [_contributor(rows, _mech('uw.spotlight.quantity'), 'edamage.spotlight_coverage'), _contributor(rows, _mech('uw.spotlight.angle_degrees'), 'edamage.spotlight_coverage')], 'EP spotlight coverage from quantity and angle [EP helper support surface]')
    _publish(rows, 'derived::edamage.super_tower_factor', super_tower_factor, 'multiplier', [_contributor(rows, _runtime('cards.super_tower.bonus_multiplier'), 'edamage.super_tower_factor')], 'EPD_SUPERTOWER_EFFECTIVE_BONUS-aligned factor [EP helper support surface]')
    _publish(rows, 'derived::edamage.rend_factor', rend_factor, 'multiplier', [_contributor(rows, _runtime('rend.lab_level'), 'edamage.rend_factor')], 'EPD_MAXREND-aligned rend factor [EP helper support surface]')
    _publish(rows, 'derived::edamage.bullet_pipeline_factor', bullet_pipeline_factor, 'scalar', [_contributor(rows, _canon('tower_attack_speed'), 'edamage.bullet_pipeline_factor')], 'EP EM5 DV5-like bullet pipeline factor [EP helper support surface]')
    _publish(rows, 'derived::edamage.spotlight_factor', spotlight_factor, 'multiplier', [_contributor(rows, _mech('uw.spotlight.damage_multiplier'), 'edamage.spotlight_factor')], 'EP spotlight final bonus with quantity and angle coverage [EP helper support surface]')
    _publish(rows, 'derived::edamage.uw.death_wave_dps', dw, 'scalar', [_contributor(rows, _mech('uw.death_wave.damage_multiplier'), 'edamage.uw.death_wave_dps')], 'EP_DW_DPS-like death wave DPS [EP helper support surface]')
    _publish(rows, 'derived::edamage.uw.chain_lightning_dps', cl, 'scalar', [_contributor(rows, _mech('uw.chain_lightning.damage_multiplier'), 'edamage.uw.chain_lightning_dps')], 'EP_CL_DPS-like chain lightning DPS [EP helper support surface]')
    _publish(rows, 'derived::edamage.uw.smart_missiles_dps', sm, 'scalar', [_contributor(rows, _mech('uw.smart_missiles.damage_multiplier'), 'edamage.uw.smart_missiles_dps')], 'smart missiles DPS or proxy [EP helper support surface]')
    _publish(rows, 'derived::edamage.uw.spotlight_missiles_dps', slm, 'scalar', [_contributor(rows, _mech('uw.spotlight_missiles.damage_multiplier'), 'edamage.uw.spotlight_missiles_dps')], 'spotlight missiles DPS or proxy [EP helper support surface]')
    _publish(rows, 'derived::edamage.uw.poison_swamp_dps', ps, 'scalar', [_contributor(rows, _mech('uw.poison_swamp.damage_multiplier'), 'edamage.uw.poison_swamp_dps')], 'poison swamp DPS or proxy [EP helper support surface]')
    _publish(rows, 'derived::edamage.uw.ilm_dps', ilm, 'scalar', [_contributor(rows, _mech('uw.inner_land_mines.damage_multiplier'), 'edamage.uw.ilm_dps')], 'EP_ILM_DPS-like inner land mine DPS including Space Displacer contribution [EP helper support surface]')
    _publish(rows, 'derived::edamage.uw_total_damage', uw_total_damage, 'scalar', [_contributor(rows, _mech('uw.chain_lightning.damage_multiplier'), 'edamage.uw_total_damage')], 'EP_UW_TOTAL_DAMAGE-like aggregate UW damage [EP helper support surface]')
    _publish(rows, 'derived::edamage.uw_crit_factor', uw_crit_factor, 'multiplier', [_contributor(rows, _canon('tower_crit_chance_pct'), 'edamage.uw_crit_factor')], 'EPD_UWCRITICAL-aligned UW crit factor [EP helper support surface]')
    _publish(rows, 'derived::edamage.slow_factor', slow_factor, 'multiplier', [_contributor(rows, _mech('uw.chrono_field.slow_pct'), 'edamage.slow_factor')], 'EP Chrono Field slow exposure multiplier [EP helper support surface]')
    _publish(rows, 'derived::edamage_ep_helper.dx_spotlight_light_range_factor', spotlight_light_range_factor, 'multiplier', [_contributor(rows, _mech('uw.spotlight.light_range'), 'edamage.ep_dx_spotlight_light_range_factor')], 'EP eDamage DX5 spotlight light-range factor alias [EP helper support surface]')
    _publish(rows, 'derived::edamage_ep_helper.dx_spotlight_bonus_term', sl_bonus, 'multiplier', [_contributor(rows, _mech('uw.spotlight.damage_multiplier'), 'edamage.ep_dx_spotlight_bonus_term')], 'EP eDamage DX5 spotlight bonus term alias [EP helper support surface]')
    _publish(rows, 'derived::edamage_ep_helper.dx_spotlight_coverage', spotlight_coverage, 'ratio', [_contributor(rows, _mech('uw.spotlight.quantity'), 'edamage.ep_dx_spotlight_coverage'), _contributor(rows, _mech('uw.spotlight.angle_degrees'), 'edamage.ep_dx_spotlight_coverage')], 'EP eDamage DX5 spotlight coverage alias [EP helper support surface]')
    _publish(rows, 'derived::edamage_ep_helper.dc_base_damage_stack', base_damage_stack, 'scalar', [_contributor(rows, _canon('tower_damage'), 'edamage_ep_helper.dc_base_damage_stack')], 'EP eDamage DC5 base damage stack alias [EP helper surface]')
    _publish(rows, 'derived::edamage_ep_helper.di_bullet_crit_factor', bullet_crit_factor, 'multiplier', [_contributor(rows, _canon('tower_crit_chance_pct'), 'edamage_ep_helper.di_bullet_crit_factor')], 'EP eDamage DI5 bullet crit factor alias [EP helper surface]')
    _publish(rows, 'derived::edamage_ep_helper.dj_uw_crit_factor', uw_crit_factor, 'multiplier', [_contributor(rows, _canon('tower_crit_chance_pct'), 'edamage_ep_helper.dj_uw_crit_factor')], 'EP eDamage DJ5 UW crit factor alias [EP helper surface]')
    _publish(rows, 'derived::edamage_ep_helper.dv_bullet_pipeline_factor', bullet_pipeline_factor, 'scalar', [_contributor(rows, _canon('tower_attack_speed'), 'edamage_ep_helper.dv_bullet_pipeline_factor')], 'EP eDamage DV5 bullet pipeline factor alias [EP helper surface]')
    _publish(rows, 'derived::edamage_ep_helper.ei_uw_total_damage', uw_total_damage, 'scalar', [_contributor(rows, _mech('uw.chain_lightning.damage_multiplier'), 'edamage_ep_helper.ei_uw_total_damage')], 'EP eDamage EI5 total UW damage alias [EP helper surface]')
    _publish(rows, 'derived::edamage_ep_helper.el_slow_factor', slow_factor, 'multiplier', [_contributor(rows, _mech('uw.chrono_field.slow_pct'), 'edamage_ep_helper.el_slow_factor')], 'EP eDamage EL5 slow factor alias [EP helper surface]')
    _publish(rows, 'derived::edamage', edamage, 'scalar', [_contributor(rows, _canon('tower_damage'), 'edamage')], 'Native eDamage objective line; currently equal to EP line')
    _publish(rows, 'derived::edamage_ep', edamage, 'scalar', [_contributor(rows, _canon('tower_damage'), 'edamage_ep')], 'Exact Effective Paths eDamage objective line')
    ...
    if cpk_factor <= 0.0:
        cpk_factor = _ep_cpk(
            _get_first(rows, [_canon('coin_kill_multiplier'), 'support_surface::eecon.coin_kill_ws'], 0.0),
    ...
    if freeup_factor <= 0.0:
        ...
        if not has_freeup or freeup_pct == 0.0:
            ...
            has_ws = _bool(rows, ['support_surface::eecon.wave_skip_active'], _get_first(rows, [_runtime('cards.wave_skip.chance_pct')], 0.0) > 0.0)
            ws_card = _get_first(rows, [_runtime('cards.wave_skip.chance_pct')], 0.0)
    ...
    if coin_card_factor <= 0.0:
        coin_card_factor = _ep_card_coins(
            _bool(rows, ['support_surface::eecon.coin_card_active'], _get_first(rows, [_runtime('cards.coins.multiplier')], 0.0) != 0.0),
            _get_first(rows, [_runtime('cards.coins.multiplier'), _mech('cards.coins.multiplier')], 1.0),
    ...
    if module_factor <= 0.0:
        module_factor = _ep_module_bonus(
            _get_first(rows, [_mech('module.generator.primary_coin_multiplier'), _mech('module.generator.coin_multiplier')], 1.0),
            _bool(rows, [_mech('module.generator.assist_enabled')], False),
            _get_first(rows, [_mech('module.generator.assist_bonus_multiplier')], 1.0),
            _get_first(rows, [_mech('module.generator.assist_stone_bonus_pct')], 0.0),
            _get_first(rows, [_mech('module.generator.assist_lab_bonus_pct')], 0.0),
        )
    ...
    has_gt = _bool(rows, ['support_surface::eecon.gt_active', _runtime('uw.golden_tower.active'), _mech('uw.golden_tower.active')], False)
    has_bh = _bool(rows, ['support_surface::eecon.bh_active', _runtime('uw.black_hole.active'), _mech('uw.black_hole.active')], False)
    has_dw = _bool(rows, ['support_surface::eecon.dw_active', _runtime('uw.death_wave.active'), _mech('uw.death_wave.active')], False)
    has_gb = _bool(rows, ['support_surface::eecon.gb_active', _runtime('uw.golden_bot.active'), _mech('uw.golden_bot.active')], False)
    ...
    sl_coin_bonus = _get_first(rows, ['support_surface::eecon.spotlight_coin_bonus_multiplier', _runtime('uw.spotlight.coin_bonus_multiplier'), _mech('uw.spotlight.coin_bonus_multiplier')], 1.0)
    sl_quantity = _get_first(rows, ['support_surface::eecon.sl_quantity'], 0.0)
    if sl_quantity <= 0.0:
        sl_quantity = _epc_slq(_get_first(rows, ['support_surface::eecon.sl_quantity_stone_level', _mech('uw.spotlight.quantity_stone_level')], _get_first(rows, [_mech('uw.spotlight.quantity')], 0.0) - 1.0))
    sl_angle = _get_first(rows, ['support_surface::eecon.sl_angle'], 0.0)
    if sl_angle <= 0.0:
        sl_angle = _epc_sla(
            _get_first(rows, ['support_surface::eecon.sl_angle_stone_level', _mech('uw.spotlight.angle_stone_level')], 0.0),
            stone_sac,
            lab_sac,
    ...
    _publish(rows, 'derived::eecon.cpk_factor', cpk_factor, 'multiplier', [_contributor(rows, _canon('coin_kill_multiplier'), 'eecon.cpk_factor')], 'EPC_CPK-like coin-per-kill factor [EP helper support surface]')
    _publish(rows, 'derived::eecon.freeup_factor', freeup_factor, 'multiplier', [], 'free-up factor [EP helper support surface]')
    _publish(rows, 'derived::eecon.coin_card_factor', coin_card_factor, 'multiplier', [_contributor(rows, _runtime('cards.coins.multiplier'), 'eecon.coin_card_factor')], 'EPC_CARD_COINS factor [EP helper support surface]')
    ...
    _publish(rows, 'derived::eecon.spotlight_coin_factor', spotlight_coin_factor, 'multiplier', [_contributor(rows, _runtime('uw.spotlight.coin_bonus_multiplier'), 'eecon.spotlight_coin_factor')], 'EP eEcon DG term [EP helper support surface]')
    ...
    _publish(rows, 'derived::eecon_ep_helper.dg_spotlight_coin_factor', spotlight_coin_factor, 'multiplier', [_contributor(rows, _runtime('uw.spotlight.coin_bonus_multiplier'), 'eecon_ep_helper.dg_spotlight_coin_factor')], 'EP eEcon DG workbook factor alias [EP helper surface]')
    _publish(rows, 'derived::eecon.wave_factor', wave_factor, 'multiplier', [], 'EP eEcon DH term [EP helper support surface]')
    _publish(rows, 'derived::eecon.base_coin_income', eecon, 'scalar', [_contributor(rows, _canon('coin_kill_multiplier'), 'eecon.base_coin_income')], 'Deterministic coin-income proxy surface derived from the governed eEcon line [Phase 3 support surface]')
    _publish(rows, 'derived::eecon', eecon, 'scalar', [_contributor(rows, _canon('coin_kill_multiplier'), 'eecon')], 'Native eEcon objective line; currently equal to EP line')
    _publish(rows, 'derived::eecon_ep', eecon, 'scalar', [_contributor(rows, _canon('coin_kill_multiplier'), 'eecon_ep')], 'Exact Effective Paths eEcon objective line')
